...
extension UIAlertController {
    ...
    convenience init(style: UIAlertController.Style, source: UIView? = nil, title: String? = nil, message: String? = nil, tintColor: UIColor? = nil) {
        ...
        if isPad {
              preferredContentSize = CGSize(width: 400, height: 300) // Adjust these values as needed
          }
        ...
        } else if isPad, let source = root{
            popoverPresentationController?.sourceView = source.view
            popoverPresentationController?.sourceRect = CGRect(x: source.view.bounds.midX, y: source.view.bounds.midY, width: 0, height: 0)
